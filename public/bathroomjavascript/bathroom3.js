...
import { planebathroom, bathroom1, bathroom2 } from '../Mainjavascript/constants.js';


const canvas = document.querySelector('.bathroom3');
...
// spotLightHelper31.layers.enable(0);
...
// spotLightHelper32.layers.enable(0);
...
// spotLightHelper33.layers.enable(0);
...
// const spotLightHelper34 = new THREE.DirectionalLight(0xffffff, 0.5);
...
// spotLightHelper35.layers.enable(0);
...
const light7 = new THREE.SpotLight(0xffffff, 1.5);
light7.position.set(-80, 420, 0);
//  light7.target.position.set(-800, 150, 0)
light7.shadow.camera.near = 0.1;
light7.shadow.camera.far = 4000;
...
const light8 = new THREE.SpotLight(0xffffff, 1.5);
light8.position.set(40, -550, 50);

scene1.add(light8);
// const spotLightHelper2 = new THREE.SpotLightHelper( light8 );
// scene1.add( spotLightHelper2 );

// RIGHT

const light9 = new THREE.SpotLight(0xffffff, 1.5);
light9.position.set(550, 0, 0);
light9.target.position.set(-900, -45, 0);


scene1.add(light9.target);
scene1.add(light9);
// const spotLightHelper2 = new THREE.SpotLightHelper( light2 );
// scene1.add( spotLightHelper2 );
...
const light19 = new THREE.SpotLight(0xffffff, 1.5);
light19.position.set(-470, 120, -150);
light19.target.position.set(800, -80, 0);
light19.castShadow = true


scene1.add(light19.target);
scene1.add(light19);
// const spotLightHelper3 = new THREE.SpotLightHelper( light19 );
// scene1.add( spotLightHelper3 );
light19.target.updateMatrixWorld();

// BACK

const light20 = new THREE.SpotLight(0xffffff, 1.5);
light20.position.set(-30, -20, 800);
light20.target.position.set(-100, 110, -350);


scene1.add(light20.target);
scene1.add(light20);
// const spotLightHelper4 = new THREE.SpotLightHelper( light20 );
// scene1.add( spotLightHelper4 );
...
let right = planebathroom.right;
let left = planebathroom.left;
let top = planebathroom.top;
let bottom = planebathroom.bottom;
let back = planebathroom.back;
let front = planebathroom.front;
...
// var defaultTexture1 = loader.load(`${'./bathroom/Screenshot 2023-08-03 140731.png'}?not-from-cache-please`);
...
// defaultTexture1.repeat.set(1, 1);
...
// defaultMaterial1.map.repeat.set(1, 1);
...
function TilesChanger(obj = {}) {
  ...
  imageload.forEach((texture, ii) => {
    ...
    const material = new THREE.MeshBasicMaterial({
      ...
      side: THREE.BackSide,
      roughness: 0.76,
      metalness: 0.9,  color: new THREE.Color(0.8, 0.8, 0.8)
    });
  ...
  const cubeGeometry = new THREE.BoxGeometry(500, 310, 880);
...
function TilesChanger2() {
  ...
  const cubeGeometry2 = new THREE.BoxGeometry(499, 309, 879);
...
// const buttonGeometry = new THREE.SphereGeometry(3, 32); // Button size
// const buttonMaterial = new THREE.MeshBasicMaterial({ color: 0xff0000 });
// const buttonMesh = new THREE.Mesh(buttonGeometry, buttonMaterial);

// // Position the button inside the box (normalized coordinates)
// buttonMesh.position.set(0, 0, 0); // Adjust the Z-coordinate to position inside the box
...
// let skyBox;
// let repeatX = 5; // Default X repeat value
// let repeatY = 4; // Default Y repeat value
// let applyChessLayout = false; // Flag to indicate whether chess layout should be applied
// let gapschange = false;
// function TilesChanger(obj = {}) {
...
//       shader.uniforms.gapColor = { value: gapColor };
//       shader.uniforms.applyChessLayout = { value: applyChessLayout };
//       shader.uniforms.gapschange = { value: gapschange };
//       shader.fragmentShader = `
//         uniform float gap;
//         uniform vec3 gapColor;
//         uniform bool applyChessLayout;
//         uniform bool gapschange;
//       ` + shader.fragmentShader;
...
//   const cubeGeometry = new THREE.BoxGeometry(430, 290, 633);
...
//   function updateMaterialShaderProperties() {
//     materialside.forEach(material => {
//       const gapColor = new THREE.Color('lightgrey');
//       material.onBeforeCompile = setMaterialShaderProperties(gapValue, gapColor, material.onBeforeCompile);
...
//   document.getElementById('gapchange').addEventListener('click', () => {
//     gapschange = !gapschange; // Toggle the chess layout flag
//     // updateMaterialShaderProperties();
//     TilesChanger();
...
// //   document.getElementById('adjustGapButton').addEventListener('click', () => {
// //     const gapInput = document.getElementById('gapInput');
// //     const newGapValue = parseFloat(gapInput.value);
// //     if (!isNaN(newGapValue)) {
// //       gapValue = newGapValue;
// //       updateMaterialShaderProperties();
// //     }
// //   });
// }
...
//   const cubeGeometry = new THREE.BoxGeometry(435, 550, 633);
...
//   const cubeGeometry2 = new THREE.BoxGeometry(504, 309, 879);
...
// let circleGeo = new THREE.CircleGeometry(110, 60);
// let circleMat = new THREE.MeshBasicMaterial({ map: new THREE.TextureLoader().load('https://jabez-tiles.s3.ap-south-1.amazonaws.com/glow1.png'), side: DoubleSide, transparent: true })
// let circle = new THREE.Mesh(circleGeo, circleMat);
// circle.position.set(-7, 90, -349);
// circle.scale.setX(1);
// scene.add(circle);


let circleGeo4 = new THREE.PlaneGeometry(120, 215);
let circleMat4 = new THREE.MeshBasicMaterial({side: THREE.DoubleSide , color:0x00000})
let circle4 = new THREE.Mesh(circleGeo4, circleMat4);
circle4.position.set(-249, 43, -52);
...
const reflectorGeometry = new THREE.PlaneGeometry(110, 205);
...
reflector.position.set(-248, 43, -52);
...
gltfloader.load('./bathroom/model (15).glb', function (gltf2) {

    const model2 = gltf2.scene;
  
    model2.rotation.y = Math.PI / 1;
    model2.scale.set(122, 122, 122);
...
// mtlLoader.load('./bathroom/Bathroom 001.mtl', materials => {
//     materials.preload();
    
//     // Load the OBJ file with the loaded materials
//     const objLoader = new OBJLoader();
//     objLoader.setMaterials(materials);
//     objLoader.load('./bathroom/Bathroom 001.obj', object => {
//         scene.add(object);
//     });
// });
...
// gltfloader.load('./bathroom/bathroom_interior.glb', function (gltf2) {

//     const model2 = gltf2.scene;
  
//     model2.rotation.y = Math.PI / 1;
//     model2.scale.set(200, 200, 200);
//     model2.position.set(732, -285, -740);
...
gltfloader.load('bathroom/pgc_steam_shower.glb', function (gltf55) {

  const model55 = gltf55.scene;

  // model55.rotation.y = Math.PI / 1;
  model55.scale.set(4.58, 3.0, 3.3);
  model55.position.set(0, -162, -280);
...
// gltfloader.load('./bathroom/bath (1).glb', function (gltf3) {
...
gltfloader.load('bathroom/japanese_toilet.glb', function (gltf4) {

  const model4 = gltf4.scene;

  model4.rotation.y = Math.PI / -2;
  model4.scale.set(15, 15, 15);
  model4.position.set(190, -160, 0);
...
// gltfloader.load('bathroom/flower vase.glb', function (gltf8) {

//   const model8 = gltf8.scene;


//   model8.scale.set(700, 700, 700);
//   model8.position.set(-170, -87, -280);
...
// gltfloader.load('bathroom/table.glb', function (gltf6) {

//   const model6 = gltf6.scene;

//   model6.rotation.y = Math.PI / 1;
//   model6.scale.set(2100, 2100, 2100);
//   model6.position.set(-3, -300, -280);
//   scene1.add(model6);

// });

// gltfloader.load('bathroom/bottle.glb', function (gltf15) {

//   const model15 = gltf15.scene;

//   model15.rotation.y = 6;
//   model15.scale.set(300, 300, 300);
//   model15.position.set(170, -90, -265);
//   scene1.add(model15);
...
async function openimage1() {
  ...
  if (index == 3 && bottomchange == 0) {
    temps = `<img  src="${filteredData[bottom].fileUrl}" width="90" height="90" style=" z-index: 4;  float:right; margin-right:20px; padding:0px; border: 4px solid white" >`
