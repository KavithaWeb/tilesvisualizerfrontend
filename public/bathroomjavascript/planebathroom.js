...
spotLightHelper34.layers.enable(0);
...
function TilesChanger(obj = {}) {
  ...
  const cubeGeometry = new THREE.BoxGeometry(495, 580, 700);
...
function TilesChanger2() {
  ...
  const cubeGeometry2 = new THREE.BoxGeometry(495, 580, 700);
...
// let circleMat = new THREE.MeshPhysicalMaterial({ side:DoubleSide, roughness:0.1, metalness:0, transmission :1 })
...
reflector.position.set(-7, 90, -344);
...
gltfloader.load('bathroom/versailles_wc_with_seat.glb', function (gltf10) {

  const model10 = gltf10.scene;

  model10.rotation.y = Math.PI / -2;
  model10.scale.set(200, 200, 200);
  model10.position.set(162, -253, 20);
...
gltfloader.load('bathroom/tapnew.glb', function (gltf4) {

  const model4 = gltf4.scene;

  model4.rotation.y = Math.PI / 1;
  model4.scale.set(200, 200, 200);
  model4.position.set(66, 60, 200);
