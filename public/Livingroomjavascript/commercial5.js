...
const canvas = document.querySelector('.commercial5');
...
let max = controls.maxDistance = 140;
...
const light5 = new THREE.SpotLight(0xffffff, 0.8);
...
console.log(texture.id);
  ...
  const cubeGeometry = new THREE.BoxGeometry(300, 200, 600);
...
//   });
  ...
  gltfloader.load('./livingroom/restaurant_street.glb', function (gltf6) {

    const model6 = gltf6.scene;
  
    model6.rotation.y = Math.PI / 1;
    model6.scale.set(30, 30, 30);
    model6.position.set(-140, -78, -200);
    scene1.add(model6);
    model6.traverse(function(child) {
      if (child.isMesh) {
        // child.castShadow = true;
