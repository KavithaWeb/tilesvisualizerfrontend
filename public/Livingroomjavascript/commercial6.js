...
const canvas = document.querySelector('.commercial6');
...
let max = controls.maxDistance = 330;
...
let gapValue = 0.003; 
 // Default gap value
let skyBox;
let repeatX = 15; // Default X repeat value
let repeatY = 20; // Default Y repeat value
...
console.log(texture.id);
  ...
  const cubeGeometry = new THREE.BoxGeometry(240, 350, 355);
...
//   });
  ...
  gltfloader.load('./livingroom/2_floor_exterior_design.glb', function (gltf6) {
    ...
    // model6.rotation.y = Math.PI / 3.75;
    model6.scale.set(15, 15, 15);
    model6.position.set(0, -70, 0);
    scene1.add(model6);
    model6.traverse(function(child) {
      if (child.isMesh) {
        // child.castShadow = true;
...
gltfloader.load('./livingroom/2021_lamborghini_countach_lpi_800-4.glb', function (gltf7) {

  const model7 = gltf7.scene;

  model7.rotation.y = Math.PI / 2;
  model7.scale.set(8, 8, 8);
  model7.position.set(60, -58, 140);
  scene1.add(model7);
