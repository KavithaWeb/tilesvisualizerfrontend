...
import { livingroom1, bathroom1, bathroom2 } from '../Mainjavascript/constants.js';
import { request } from '../services/api.service.js';


const canvas = document.querySelector('.livingroom2');
...
// // Function to smoothly animate the camera position
...
//       camera.position.z = targetZ; // Set the final position
//       cancelAnimationFrame(animationId); // Stop the animation
//       return;
//     }

//     const progress = elapsedTime / animationDuration;
//     const newPosition = startZ + (targetZ - startZ) * progress;
...
//     const targetZ = camera.position.z - zoomStepin; // Calculate the target zoom position
...
//     const targetZ = camera.position.z + zoomStepout; // Calculate the target zoom position
//     animateCamera(targetZ);
//   }
// }
...
let right = livingroom1.right;
let left = livingroom1.left;
let top = livingroom1.top;
let bottom = livingroom1.bottom;
let back = livingroom1.back;
let front = livingroom1.front;
...
function renderData(start) {
    ...
    const attachedData1 = { fileUrl: "https://jabez-tiles.s3.ap-south-1.amazonaws.com/PW03540_b.jpg", id:"11", tiles: "tilesimages" };
    const newData1 = attachedData1;
  
    const attachedData2 = { fileUrl: "https://jabez-tiles.s3.ap-south-1.amazonaws.com/joshua-bartell-6vvIBTvL90A-unsplash.jpg", id:"12", tiles: "tilesimages" };
...
var defaultTexture22 = loader.load(`${'https://jabez-tiles.s3.ap-south-1.amazonaws.com/WoodFine0058_30_350.jpg'}?not-from-cache-please`);
...
defaultTexture22.repeat.set(22, 2);
...
const defaultGap = 0.004;
const defaultGapColor = new THREE.Color('black');
...
defaultMaterial22.map.repeat.set(22, 2);
...
console.log(texture.id);
  ...
  const cubeGeometry = new THREE.BoxGeometry(2000, 890, 1250);
...
function TilesChanger2(obj = {}) {
  ...
  var imageload3 = loader.load(`${'https://jabez-tiles.s3.ap-south-1.amazonaws.com/Screenshot 2023-04-24 203442.png'}?not-from-cache-please`)
    const material1 = new THREE.MeshStandardMaterial({ map: imageload2, side: BackSide, transparent: true, opacity: 0.7 });
    const material2 = new THREE.MeshStandardMaterial({ map: imageload3, side: BackSide, transparent: true, opacity: 0.08 });

let materialarray = [material1, material2];

const cubeGeometry2 = new THREE.BoxGeometry(1999, 888, 1248);
...
// let circleMat1 = new THREE.MeshStandardMaterial({color:0xFFD700, side:THREE.DoubleSide});
...
// gltfloader.load('./livingroom1/ac.glb', function (gltf6) {

//   const model6 = gltf6.scene;

//   // model6.rotation.y = Math.PI / 1;
//   model6.scale.set(2500, 2500, 2500);
//   model6.position.set(950, -350, -450);
//   scene1.add(model6);
...
// gltfloader.load('./livingroom1/lamp (2).glb', function (gltf9) {

//   const model9 = gltf9.scene;

//   // model9.rotation.y = Math.PI / 1;
//   model9.scale.set(200, 200, 200);
//   model9.position.set(-700, 60, -570);
//   scene1.add(model9);0
...
gltfloader.load('./livingroom1/sofa 2.glb', function (gltf8) {

  const model8 = gltf8.scene;

  // model8.rotation.y = Math.PI / 1;
  model8.scale.set(3400, 3000, 3000);
  model8.position.set(1290, -380, -480);
  scene1.add(model8);
...
gltfloader.load('./livingroom1/sofa set.glb', function (gltf5) {

  const model5 = gltf5.scene;

  model5.rotation.y = Math.PI / -1;
  model5.scale.set(3000, 3000, 2000);
  model5.position.set(-100, -380, -220);
  scene1.add(model5);
...
gltfloader.load('./livingroom1/sofa set.glb', function (gltf10) {

  const model10 = gltf10.scene;

  model10.rotation.y = Math.PI / -1;
  model10.scale.set(3000, 3000, 2000);
  model10.position.set(-1200, -380, -220);
  scene1.add(model10);
...
gltfloader.load('https://jabez-tiles.s3.ap-south-1.amazonaws.com/photos.glb', function (gltf25) {

  const model25 = gltf25.scene;

  model25.rotation.y = Math.PI / -2;
  model25.scale.set(800, 800, 800);
  model25.position.set(438, -90, -335);
  scene1.add(model25);
...
// gltfloader.load('./livingroom1/olddoor1.glb', function (gltf11) {
...
// gltfloader.load('./livingroom1/door.glb', function (gltf11) {
...
gltfloader.load('./livingroom1/plant (1).glb', function (gltf12) {

  const model12 = gltf12.scene;

  model12.rotation.y = Math.PI / -1;
  model12.scale.set(800, 820, 800);
  model12.position.set(830, -320, -920);
  scene1.add(model12);
...
// gltfloader.load('./livingroom1/wall light.glb', function (gltf13) {
...
// gltfloader.load('https://jabez-tiles.s3.ap-south-1.amazonaws.com/tables.glb', function (gltf13) {

//   const model13 = gltf13.scene;

//   // model13.rotation.y = Math.PI / 1;
//   model13.scale.set(4000, 4000, 4000);
//   model13.position.set(1530, -360, -580);
//   scene1.add(model13);
//   model13.traverse(function(child) {
//     if (child.isMesh) {
//       // child.castShadow = true;
...
gltfloader.load('https://jabez-tiles.s3.ap-south-1.amazonaws.com/sofa table2.glb', function (gltf18) {

  const model18 = gltf18.scene;

  model18.rotation.y = Math.PI / 1;
  model18.scale.set(2800, 2800, 2800);
  model18.position.set(200, -350, 330);
  scene1.add(model18);
...
// gltfloader.load('./livingroom1/bulbs 2.glb', function (gltf14) {
...
gltfloader.load('./livingroom1/uploads_files_3427637_Lamp.gltf', function (gltf17) {
