...
const canvas = document.querySelector('.commercial2');
...
let max = controls.maxDistance = 150;
...
console.log(texture.id);
  ...
  const cubeGeometry = new THREE.BoxGeometry(910, 390, 780);
...
const cubeGeometry2 = new THREE.BoxGeometry(909, 389, 959);
...
var tex = textureLoader8.load('./livingroom/riyadvilogo.png');
let circleGeo2 = new THREE.PlaneGeometry(70,50);
let circleMat2 = new THREE.MeshBasicMaterial({map:tex, transparent:true});
let circle2 = new THREE.Mesh(circleGeo2, circleMat2);
circle2.position.set(-302 ,132 ,-0);
...
let circleGeo6 = new THREE.PlaneGeometry(100,200);
...
gltfloader.load('./livingroom/sketchfab_store_in_mall.glb', function (gltf6) {

    const model6 = gltf6.scene;
  
    model6.rotation.y = Math.PI / 1;
    model6.scale.set(2, 2, 2);
    model6.position.set(0, -100, 10);
    scene1.add(model6);
    model6.traverse(function(child) {
      if (child.isMesh) {
        // child.castShadow = true;
