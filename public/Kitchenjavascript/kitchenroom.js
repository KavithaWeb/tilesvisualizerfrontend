...
import { kitchenback, kitchenback1, bathroom2 } from '../Mainjavascript/constants.js';
...
let max = controls.maxDistance = 390;
...
let right = kitchenback.right;
let left = kitchenback.left;
let top = kitchenback.top;
let bottom = kitchenback.bottom;
let back = kitchenback.back;
let front = kitchenback.front;




let right2 = kitchenback1.right2;
let left2 = kitchenback1.left2;
let top2 = kitchenback1.top2;
let bottom2 = kitchenback1.bottom2;
let back2 = kitchenback1.back2;
let front2 = kitchenback1.front2;
...
// var images3 = document.getElementsByClassName("img2");
// let imageload3 = []
// for (const img3 of images3) {
//   imageload3.push(loader.load(`${img3.src}?not-from-cache-please`))
// }
...
function renderData(start) {
   ...
   filteredData = data.filter((item) => item.tiles === "tilesimages" );
    ...
    const attachedData3 = { fileUrl: "https://jabez-tiles.s3.ap-south-1.amazonaws.com/PF01965_b.jpg", id:"11", tiles: "tilesimages" };
...
function TilesChanger(obj = {}) {
  ...
  const cubeGeometry = new THREE.BoxGeometry(1460, 260, 1250);
  ...
  const groups = [
    ...
    { start: 30, count: 6, materialIndex: front }
  ];
...
let skyBox2;
 ...
 // materialside.push(defaultMaterial3);
  ...
  const cubeGeometry2 = new THREE.BoxGeometry(1460, 630, 1250);
...
//   model7.scale.set(6520, 7970, 6320);
//   model7.position.set(300, -390, -80);
//   scene1.add(model7);
...
async function openimage1(){
  ...
  const url = `https://newtiles-env.eba-mdsv5qca.ap-south-1.elasticbeanstalk.com/users`;
...
if(index == 0 && topchange == 2){
  temps = `<img  src="${filteredData[front2].fileUrl}" width="90" height="90" alt="Snow" style=" z-index: 4; float:right; margin-right:20px; padding:0px; border: 4px solid white;" >`
...
for (const img of images) {
  img.addEventListener('click', function oClick() {
   ...
   function myURL() {
    document.getElementById('logout1-btn-loader').style.display = "none";
      TilesChanger({ front: parseInt(img.name) });
   ...
   function myURL() {
    document.getElementById('logout1-btn-loader').style.display = "none";
    // TilesChanger({ right: parseInt(img.name) });
    TilesChanger2({ front2: parseInt(img.name) });
    // TilesChanger2({ right2: parseInt(img.name) });
   }
 }
